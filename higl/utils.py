...
class ReplayBuffer(object):
    ...
    # Expects tuples of (x, x', ag, ag', g, u, r, d, x_seq, a_seq, ag_seq)
    def add(self, odict):
    ...
    def get_stats(self):
        states = torch.tensor(np.array(self.storage[0]))
        return states.mean(dim=0), states.std(dim=0)

    def sample(self, batch_size):
        ind = np.random.randint(0, len(self.storage[0]), size=batch_size)
        # print(np.array(self.storage[0]).shape)
    ...
    def save(self, file):
        np.savez_compressed(file, idx=np.array([self.next_idx]), x=self.storage[0],
                            y=self.storage[1], ag=self.storage[2], ag_next=self.storage[3], g=self.storage[4], u=self.storage[5],
                            r=self.storage[6], d=self.storage[7], xseq=self.storage[8],
                            aseq=self.storage[9], ag_seq=self.storage[10])

    def load(self, file):
        with np.load(file) as data:
            self.next_idx = int(data['idx'][0])
            self.storage = [data['x'], data['y'], data['ag'], data['ag_next'], data['g'], data['u'], data['r'],
                            data['d'], data['xseq'], data['aseq'], data['ag_seq']]
...
class NormalNoise(object):
    ...
    def perturb_action_batch(self, action, min_action=-np.inf, max_action=np.inf):
        action = (action + np.random.normal(0, self.sigma,
